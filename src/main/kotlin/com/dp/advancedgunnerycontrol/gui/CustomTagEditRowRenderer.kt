...
internal object CustomTagEditRowRenderer {
    ...
    fun addMomentaryRow(
        ...
        val control = buttons.addCampaignButtonControl(button = button) {
            if (enabled) {
                onClick()
            }
        }
        if (!enabled) {
            control.applyDisabledCampaignButtonTemplate(showTooltipWhileInactive = true)
        }
    }
    ...
    fun addBidirectionalMomentaryRow(
        ...
        val control = buttons.addBidirectionalCampaignMomentaryButton(
            button = button,
            onLeftClick = {
                if (enabled) {
                    onLeftClick()
