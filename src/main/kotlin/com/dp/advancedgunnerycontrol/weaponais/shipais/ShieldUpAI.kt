...
class ShieldUpAI(ship: ShipAPI, private val fluxThreshold: Float) : ShipCommandGenerator(ship) {
    ...
    override fun generateCommands(): List<ShipCommandWrapper> {
        if (!areEnemiesInRange()) return emptyList()
        if ((ship.fluxTracker?.fluxLevel ?: 1f) > fluxThreshold) return emptyList()
        return if (ship.shield?.isOff == true) {
            listOf(ShipCommandWrapper(ShipCommand.TOGGLE_SHIELD_OR_PHASE_CLOAK))
        } else {
            emptyList()
        }
    }
