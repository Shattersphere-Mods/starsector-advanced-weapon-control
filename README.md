# Advanced Gunnery Control - Starsector Mod #
...
Priority tags do not force a weapon to fire, and they do not make invalid targets valid. They make matching targets more
attractive when AGC is choosing between possible targets. Internally, lower priority scores are better.

AGC's custom AI already has a strong bias toward the selected ship target, the previous target, and targets near the
weapon's current facing and range. `Prio...` tags multiply the score for matching targets, making those targets more
likely to win that choice. A larger editable priority multiplier means a stronger preference, not a hard rule. Use
`Target...`, `No...`, `Avoid...`, `Hold...`, or `Force...` tags when you want stricter behaviour than "prefer this if
possible".
...
|   NoShield   | Targets without active shields | Targets without active shields                       | None          |     Base AI targets shields     |                    This is simple boolean logic. If a target has no shields or its shields are turned off, it will get targeted. Legacy `ShieldOff` and `ShieldsOff` are still accepted.                     |                                other shield modes                                 | Was requested for EMP weapons, so EMP weapons I guess?                                   | No                             |
